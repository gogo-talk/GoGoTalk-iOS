...
@implementation NSString (XCHelper)
...
- (NSString *)xc_to16MD5 {
    ...
    return [[self xc_toMD5] substringWithRange:NSMakeRange(8, 16)];
}
...
- (BOOL)xc_isEmail {
    return [NSString xc_isEmail:self];
}
...
- (BOOL)xc_isMobilePhone {
    return [NSString xc_isMobilePhone:self];
}
...
- (BOOL)xc_isTelNumber {
    return [NSString xc_isTelNumber:self];
}
...
- (BOOL)xc_isPersonID {
    return [NSString xc_isPersonID:self];
}
...
+ (BOOL)xc_isPersonID:(NSString *)PID {
    ...
    if (PID.length == 15) {
        ...
        int o = p % 11;
        NSString *xc_content = [NSString stringWithFormat:@"%c", sChecker[o]];
        [mString insertString:xc_content atIndex:[mString length]];
        carid = mString;
    }
...
- (NSString *)xc_trim {
    NSString *str = [self xc_trimLeft];
    return [str xc_trimRight];
}
